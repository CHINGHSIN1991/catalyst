...
const PanelOpenBackground = styled.div<panelState>`
  width: 100vw;
...
const PanelContainer = styled.div<panelState>`
...
const AlertWindow: React.FC<{}> = () => {
  ...
  return (
    <PanelOpenBackground
      editPanelState={alertState.title}
      onClick={() => setAlertState({ title: '' })}
    >
  ...
  return (
    ...
    >
      <EditPanelTitle>
        ...
        <AlertTitleText>{alertState.title}</AlertTitleText>
        <EditPanelTitleUnderLine></EditPanelTitleUnderLine>
      </EditPanelTitle>
      <InfoContainer>{alertState.message}</InfoContainer>
      {!alertState.function && (
        <ButtonContainer>
          <PanelButton
            width={96}
