...
export const CurrentFocusPanel: React.FC<{}> = () => {
  ...
  function checkIsCurrent(items: calendarItem[]) {
    ...
    let tempItems = sortByTimeStamp(
      items.filter((item) => getTimeStamp(item.end, 'end') > current),
      'start'
    )
    const onGoing = []
    const upComing = []
  ...
  function checkUpdate() {
    ...
    if (
      currentTask.current &&
      taskOnGoing &&
      getTimeStamp(currentTask.current.end, 'end') < current
    ) {
      getCalendarEvents()
    } else if (
      currentTask.current &&
      !taskOnGoing &&
      getTimeStamp(currentTask.current.start, 'start') < current
    ) {
      getCalendarEvents()
    }
  }
    ...
    <Wrapper>
      <FocusBlock>
        {calendarItem &&
          taskOnGoing !== null &&
          taskOnGoing &&
          `Current focus: ${calendarItem.summary}（till ${getTimeString(
            calendarItem.end,
            'end'
          )}）`}
        {calendarItem &&
          taskOnGoing !== null &&
          !taskOnGoing &&
          `Upcoming: ${calendarItem.summary}（start at ${getTimeString(
            calendarItem.start,
            'start'
          )}）`}
      </FocusBlock>
