...
const TextNote = styled.div<scheme>`
  height: 0px;
  padding-top: 0px;
  font-size: 0.75rem;
  line-height: 1rem;
...
const ToDoContent = styled.div<scheme>`
  display: flex;
  align-items: flex-start;
  color: ${(props) => props.theme.primary};
`

const TextContent = styled.div<scheme>`
  color: ${(props) => props.theme.primary};
  flex-grow: 1;
  width: auto;
  overflow: hidden;
...
const TextTitle = styled.div<isDone & scheme>`
  font-size: 0.875rem;
  line-height: 1rem;
  color: ${(props) =>
    props.isDone ? props.theme.tertiary : props.theme.primary};
...
const AddToDoContainer = styled.div<isEditOn>`
  border-radius: 4px;
  background-color: rgba(255, 255, 255, 0.1);
  width: 100%;
  height: ${(props) => (props.isEditOn ? '86px' : '0px')};
  overflow: hidden;
  display: flex;
  flex-direction: column;
...
const ToDoInput = styled.input<scheme>`
  height: 24px;
  padding: 0 4px;
  color: ${(props) => props.theme.primary};
...
const ToDoContainer = styled(ScrollbarContainer)<isEditOn>`
  width: 100%;
  transition: 0.2s;
  height: ${(props) =>
    props.isEditOn ? 'calc(100vh - 836px)' : 'calc(100vh - 750px)'};
  @media (max-width: 1580px) {
    height: ${(props) =>
      props.isEditOn ? 'calc(100vh - 614px)' : 'calc(100vh - 528px)'};
  }
  @media (max-width: 1180px) {
    height: ${(props) =>
      props.isEditOn ? 'calc(100vh - 428px)' : 'calc(100vh - 342px)'};
  }
`
    ...
    <WorksPanel>
      ...
      {!tempTodo.id && (
        <AddToDoPanel
          ...
          editTodo={editTodo}
          handleIsSetAlert={handleIsSetAlert}
        ></AddToDoPanel>
      )}
...
const ToDoElement: React.FC<{
  ...
  return (
    ...
    >
      <ToDoContent>
        {props.tempTodo.id !== props.item.id && (
          <CheckContainer
            onClick={() => {
        ...
        {props.tempTodo.id === props.item.id && (
          <CheckContainer
            onClick={() =>
              props.setTempTodo({
                ...props.tempTodo,
                isSetAlert: !props.tempTodo.isSetAlert,
              })
            }
          >
            {props.tempTodo.isSetAlert && (
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
        ...
        {props.tempTodo.id !== props.item.id && (
          ...
          >
            <TextTitle isDone={props.item.isDone}>
              {props.item.workContent}
            </TextTitle>
            {props.item.isSetAlert && (
              <TextNote>{`Alarm : ${props.item.alertDate} ${props.item.alertTime}`}</TextNote>
            )}
          </TextContent>
        )}
        {props.tempTodo.id === props.item.id && (
          <TextContent>
            <TitleEdit
              style={{ width: '224px' }}
              value={props.tempTodo.workContent}
              name="workContent"
              onChange={(e) =>
                handleInputChange(e, props.tempTodo, props.setTempTodo)
              }
              type="text"
            ></TitleEdit>
            {props.tempTodo.isSetAlert && (
              <ToDoLabel htmlFor="">
                <ToDoInput
                  ...
                  value={props.tempTodo.alertDate || ''}
                  type="date"
                />
                <ToDoInput
                  name="alertTime"
                  ...
                  value={props.tempTodo.alertTime || ''}
                  type="time"
                />
              </ToDoLabel>
            )}
        ...
        {props.tempTodo.id === props.item.id && (
          <CompleteBtns>
            ...
            <Btn
              onClick={() => {
                setIsEditOn(false)
                props.setTempTodo({ id: 0, workContent: '', isSetAlert: false })
              }}
            >
              Cancel
            </Btn>
