...
const EditPanel: React.FC<{}> = () => {
  ...
  return (
    <PanelOpenBackground
      editPanelState={editPanelState.name}
      onClick={() => dispatch(setEditPanel({ name: '', data: '' }))}
    >
      <PanelContainer editPanelState={editPanelState.name}>
        {(editPanelState.name === 'ShortcutEdit' ||
          editPanelState.name === 'ShortcutAdd') && (
          <ShortcutEditPanel></ShortcutEditPanel>
        )}
        {(editPanelState.name === 'EventEdit' ||
          editPanelState.name === 'EventAdd') && (
          <CalendarEditPanel></CalendarEditPanel>
        )}
        {editPanelState.name === 'BackgroundEdit' && (
          <BackgroundEditPanel></BackgroundEditPanel>
        )}
        {editPanelState.name === 'UserInfoEdit' && (
          <UserInfoEditPanel></UserInfoEditPanel>
        )}
        {editPanelState.name === 'PronounceToolEdit' && (
          <PronounceToolEditPanel></PronounceToolEditPanel>
        )}
        {editPanelState.name === 'MenuOptionEdit' && (
          <MenuOptionPanel></MenuOptionPanel>
        )}
        {editPanelState.name === 'DisplayModeEdit' && (
          <DisplayModePanel></DisplayModePanel>
        )}
        {editPanelState.name === 'ServiceEdit' && (
          <ServiceEditPanel></ServiceEditPanel>
        )}
      </PanelContainer>
    </PanelOpenBackground>
  )
