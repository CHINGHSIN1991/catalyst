...
export const InspirationNotePanel: React.FC<{}> = () => {
  ...
  function deleteTag(deletedTag: string) {
    const tempNoteCategories = [...noteCategories].filter(
      (item) => item !== deletedTag
    )
    ...
    chrome.storage.sync.set(
      {
        ...
        inspirationNotes: tempInspirationNotes,
      },
      function () {
  ...
  function loadNotes() {
    chrome.storage.sync.get(
      ['inspirationNotes', 'noteCategories'],
      (result) => {
      ...
      <ScrollContainer shortcutNumber={ShortcutNumber.length}>
        {inspirationNotes && 'no category' in inspirationNotes && (
          <TempLinks length={inspirationNotes['no category'].length}>
            {inspirationNotes['no category'] &&
              inspirationNotes['no category'].map((note: inspirationNote) => {
                return (
                  <TempLinkElement
                    key={note.id}
                    note={note}
        ...
        {noteCategories &&
          ...
          noteCategories.map((category, index) => {
            return (
              <TempLinks
                key={category + index}
                length={inspirationNotes[category].length}
              >
                <DeleteTagButton onClick={() => checkToDeleteTag(category)}>
                  ╳ Delete
                </DeleteTagButton>
                <CategoryTitle>
                  {inspirationNotes[category] &&
                    !!inspirationNotes[category].length &&
                    category}
                </CategoryTitle>
                <CategoryHr></CategoryHr>
                {inspirationNotes[category] &&
                  inspirationNotes[category].map((note: inspirationNote) => {
                    return (
                      <TempLinkElement
                        key={note.id}
                        note={note}
    ...
    >
      <LinkContent>
        ...
        {props.tempNote.id !== props.note.id && (
          <TextContent href={props.note.url} target="_blank">
