...
export const CalendarEditPanel: React.FC<{}> = () => {
  ...
  function postEvent() {
    postNewEvent(userInfo, tempEvent).then((res) => {
      ...
      fetchCalendarData(
        ...
        dayEnd,
        userInfo.authToken
      ).then((res) => dispatch(loadEvents(res.items)))
    })
  }
  ...
  return (
    ...
    >
      <EditPanelTitle>
        <EditPanelTitleText>Add event</EditPanelTitleText>
      ...
      <SwitchContainer>
        ...
        <Switch
          onClick={() => {
            setTempEvent({ ...tempEvent, isAllDay: !tempEvent.isAllDay })
          }}
          isAllDay={tempEvent.isAllDay}
        >
          <SwitchOption isAllDay={tempEvent.isAllDay} />
        </Switch>
      </SwitchContainer>
      <FormSet>
            ...
            calendarColorList.map((item) => {
              return (
                ...
                <ColorOption
                  ...
                  item={item}
                  onClick={() =>
                    setTempEvent({ ...tempEvent, colorId: item.colorId })
                  }
                  tempEvent={tempEvent}
                />
              )
        ...
        <PublicOptionSet
          onClick={() =>
            setTempEvent({
              ...tempEvent,
              visibility:
                tempEvent.visibility === 'public' ? 'private' : 'public',
            })
          }
        >
          <PublicOptionBg
            isPublic={tempEvent.visibility === 'public'}
          ></PublicOptionBg>
          <PublicOption isPublic={tempEvent.visibility === 'public'}>
            Public
          </PublicOption>
          <PublicOption isPublic={tempEvent.visibility === 'private'}>
            Private
          </PublicOption>
        </PublicOptionSet>
      </FormSet>
